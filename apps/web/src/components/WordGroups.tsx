...
export function WordGroups({ words, className = "" }: { words: string[]; className?: string }) {
...
function WordLengthSection({ group }: { group: WordLengthGroup }) {
  return (
    <View className="mb-3">
      <Text className="mb-1 font-display text-sm text-primary">{lengthLabel(group.length)}</Text>
      <Text className="font-body text-muted-foreground">{group.words.join(", ")}</Text>
    </View>
  );
}
