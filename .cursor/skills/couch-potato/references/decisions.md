...
- **Couch jam nudge:** quiet muted strip under Customize (BGM for lobby + play) — never a potato hero banner above mode cards. Player label is **Couch jam**, not Lobby jam.
- **About:** chrome About icon + ⌘K → credits modal (free + open source; X `@yo_puaaa`; GitHub repo `MattPua/A-Couch-Potato`) — not a full route. Hero = `PotatoWaveSvg` — purpose-built PixelLab `logo-wave` when present; cheer + soft rock/sparkles until then. Never a glued-on SVG hand.
