...
- Mobile-first; play/results = centered narrow `max-w-md` column, always. **Desktop real estate:** chrome pages (lobby `cp-shell-lobby`, Couch crew `cp-shell-crew`, and similar) widen + go multi-col on md+ instead of sitting as a lonely narrow stick — see `.cursor/rules/ui.mdc`
...
- **Achievements ("Couch medals")** — local per-profile progression, `apps/web/src/achievements.ts` owns all track/milestone math (UI never recomputes). Staged tracks (ascending milestones, fast-then-meatier): lifetime **Points haul**, **Word collector** (unique words), **Couch sessions** (runs completed, from `Profile.gamesPlayed` — not duplicated in the achievements blob), length hauls (**3/4/5/6/7+-letter finds**, occurrences not unique), single-run records **Best haul** + **Word rush**, and Survival hooks **Longest survival** + **Survival hauls** (populate once `RunHaul.mode === "survival"`). Persisted on `Profile.achievements` (`AchievementCounts`, migrates safely via `normalizeAchievementCounts`); `AchievementContext` = counts + injected `gamesPlayed` for progress math/rendering (`withGamesPlayed`). `storage.ts#recordFinishedRun` applies each run (`applyRunToAchievements`) and returns a snapshot + `StageUp[]` + touched track ids. Results page shows a "Couch medals" peek (`ResultsMedals`, stage-ups first) when anything moved; full breakdown lives at `/achievements` (linked from lobby, Couch crew, and results), grouped into "The big picture" / "Personal bests" / "Length hauls" / "Survival".
