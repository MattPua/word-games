...
- **Home setup = game lobby** (not a settings form): **Primary** — mode cards, board picker (size + Square/Honeycomb), challenge (How hard? / How long?). **Advanced** (quieter fine-tune) — Min length choice cards (helper + 3+/4+/5+ hints), Show words left Switch, **Dark mode** Switch (default system, explicit light/dark override). Phone: stack primary then advanced. Wide web: primary left | advanced right; cards hug content. Play dominates; sticky Play bar = Play + background music + SFX only (no theme toggle, not important enough to compete with Play); narrows on md+ lobby (centered cluster, not full shell width). See `.cursor/rules/ui.mdc`.
